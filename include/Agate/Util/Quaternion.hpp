...
{
public:
    Quaternion() { q[0] = q[1] = q[2] = q[3] = 0.0; }
    ...
    Quaternion conjugate() { return {q[0], -q[1], -q[2], -q[3]}; }
    ...
    // l2 norm
    float norm() const { return sqrtf(q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3]); }
...
{
    float inverse = 1.0f / a;
    return {r[0] * inverse, r[1] * inverse, r[2] * inverse, r[3] * inverse};
}
...
inline bool operator!=(const Quaternion& l, const Quaternion& r) { return !(l == r); }
